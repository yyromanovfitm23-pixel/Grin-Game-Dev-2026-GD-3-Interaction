...
namespace Assets.HeroEditor.Common.Scripts.EditorScripts
{
    ...
    public class CharacterEditor : CharacterEditorBase
    {
	    ...
	    {
            StartCoroutine(StandaloneFilePicker.SaveFile("Save as JSON", "", "Character", ".json", Encoding.Default.GetBytes(Character.ToJson()), (success, path) => { Debug.Log(success ? $"Saved as {path}" : "Error saving."); }));
		}
	    ...
	    {
            StartCoroutine(StandaloneFilePicker.OpenFile("Open as JSON", "", ".json", (success, path, bytes) => { if (success) Character.FromJson(Encoding.Default.GetString(bytes)); }));
	    }
